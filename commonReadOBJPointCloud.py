...
def loadJSON(data):
    ...
    for row in data['vertices']:
        v = Vertex(
            index=indexCounter,
            position=np.asarray([row[0], row[1], row[2]]),
            delta=[],
    ...
    for row in data['faces']:
        f = Face(
            index=indexCounter,
            verticesIndices=[int(row[0]), int(row[1]), int(row[2])],
            verticesIndicesSorted=[int(row[0]), int(row[1]), int(row[2])].sort(),
            vertices=[],
...
def NumpyArrayToPointCloudStructure(inputArray, nn=6, simplify=None):
    vertices = []
    normals = []
    faces = []
    edges = []
    adjacency = []
    print("Execute... loadPointCloudFromNumpyArray")
    for i in range(np.shape(inputArray)[0]):
        v = Vertex(
            index=i,
            position=inputArray[i, :],
            delta=[],
            normal=np.asarray([0.0, 0.0, 0.0]),
            neighbouringFaceIndices=[],
            neighbouringVerticesIndices=[],
    ...
    if simplify is not None:
        vertices = random.sample(vertices, int((1 - simplify) * len(vertices)))
...
def loadObjPointCloudFileToNumpyArray(filename, nn=6, simplify=None):
    vertices = []
    with open(filename, newline='') as f:
        flines = f.readlines()
        # Read vertices
        ...
        for row in flines:
            if row[0] == 'v' and row[1] == ' ':
                ...
                coords = line.split()
                coords = list(map(float, coords))
                vertices.append(np.asarray([coords[0], coords[1], coords[2]]))
        if simplify is not None:
            vertices = random.sample(vertices, int((1 - simplify) * len(vertices)))
...
def loadObjPC(filename, nn=6, simplify=None):
    vertices = []
    normals = []
    faces = []
    edges = []
...
def exportObj(Geom, filename, color=False, vertexNormals=False):
    F = []
    for f in Geom.faces:
        if len(f.verticesIndices) == 3:
    ...
    with open(filename, 'w') as writeFile:
        ...
        if vertexNormals:
            for v in Geom.vertices:
                line = "n " + str(v.normal[0]) + " " + str(v.normal[1]) + " " + str(v.normal[2])
                writeFile.write(line)
                writeFile.write('\n')
...
def exportPLYPC(Geom, filename):
    ...
    with open(filename, 'w') as writeFile:
        ...
        writeFile.write('\n')
        for j in range(0, np.size(N, axis=0)):
            line = str(round(V[j, 0], ndigits=4)) + " " + str(V[j, 1]) + " " + str(V[j, 2]) + " " + str(
                N[j, 0]) + " " + str(N[j, 1]) + " " + str(N[j, 2])
            writeFile.write(line)
            writeFile.write('\n')
...
def neighboursByVertex(Geom, vertexIndex, numOfNeighbours, useRings=True):
    ...
    patchFaces = cropList(patchVertices, numOfNeighbours)
...
def computePointCloudNormals(Geom, nNeigb):
    ...
    reportingStep = int(V.shape[0] / 10.0)
    for i in range(V.shape[0]):
        if i % reportingStep == 0:
    ...
    adjList = [0]
    for i in adjList:
        # for nn in nearest_ind[i, :].tolist()[:64]:
...
def computeThetasV3(mModel, rModel, m2R):
    thetas = np.zeros((len(mModel.faces), 1))
    for f_index, f in enumerate(mModel.faces):
        nr = rModel.faces[f_index].faceNormal
        nm = mModel.faces[m2R[f_index]].faceNormal
        thetas[f_index] = 180 * (np.arccos(np.dot(nr, nm) / (np.linalg.norm(nr) * np.linalg.norm(nm))) / np.pi)
    return thetas
