...
from configPredictSaliency02FCN import *
...
print('Compile model')
saliency_model.compile(loss=keras.losses.MeanSquaredError(), optimizer='adam', metrics=[keras.metrics.RootMeanSquaredError()])
print('Load weights')
...
print('Start')
subset = random.sample(list(range(iLen)), int(0.2 * iLen))
subset.sort()
for ipointer in range(0, len(subset), batchsize):
    x_batch = []
    x_batch_index_correspondence=[]
    istart = ipointer
    iend = min(ipointer + batchsize, len(subset) - 1)
    subsetSection=subset[istart:iend]
    for i in subsetSection:
        if mode == "MESH":
            patchIndices=neighboursByFace(mModel, i, numOfElements)[0]
            patchFacesOriginal = [mModel.faces[i] for i in patchIndices]
            ...
            ph = np.zeros((patchSide, patchSide))
            ph = np.asarray(patchIndices).reshape((patchSide, patchSide, 1))
            ...
            x_batch.append(pIn)
            x_batch_index_correspondence.append(ph)
        if mode == "PC":
            patchIndices = neighboursByVertex(mModel, i, numOfElements)[0]
            patchVerticesOriginal = [mModel.vertices[i] for i in patchIndices]
            # patchFacesOriginal = [mModel.faces[i] for i in mModel.faces[i].neighbouringFaceIndices64[:numOfElements]]
    ...
    x_batch_index_correspondence=np.asarray(x_batch_index_correspondence).astype(int)

    # if np.shape(x_batch) == (batchsize, patchSide, patchSide, 3):
    if True:
        result = saliency_model.predict(x_batch)
        for r_b in range(np.shape(result)[0]):
            inferenceReshaped = np.zeros(numOfElements)
            phReshaped = np.zeros(numOfElements)
            for r_i in range(np.shape(HC2I)[0]):
                for r_j in range(np.shape(HC2I)[1]):
                    ind = HC2I[r_i, r_j]
                    inferenceReshaped[ind] = result[r_b, r_i, r_j, 0]
                    phReshaped[ind] = x_batch_index_correspondence[r_b, r_i, r_j, 0]
            phReshaped = phReshaped.astype(int)

            for r_i in range(min(20, numOfElements)):
                prediction[phReshaped[r_i]] = inferenceReshaped[r_i]
    # a = result[0].tolist()
    # r = 0
    ...
    # for result_i in range(np.shape(result)[0]):
    #     for result_j in range(np.shape(result)[1]):
    #         for result_k  in range(np.shape(result)[2]):
    #             prediction[pallIndices[result_i,result_j,result_k]]=result[result_i,result_j,result_k]
...
modelToSave=modelName + "_" + reshapeFunction + "_pred" + "_fcn_" + "p_" + str(numOfElements)
